...
private:
    ...
    GLuint vao, vbo_Position, mvpMatrixUniform; // OpenGL objects
    mat4 orthographicProjectionMatrix; // Projection matrix
...
OpenGLApp::OpenGLApp() : ghwnd(NULL), ghdc(NULL), ghrc(NULL), gpFile(NULL), shaderProgramObject(0),
                         vao(0), vbo_Position(0), mvpMatrixUniform(0), gbFullscreen(FALSE), gbActive(FALSE), dwStyle(0), wpPrev({sizeof(WINDOWPLACEMENT)})
{
