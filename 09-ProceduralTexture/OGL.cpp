...
private:
    ...
    static const unsigned int checkImageWidth = 64;           // Texture dimensions
    static const unsigned int checkImageHeight = 64;          // Texture dimensions
    GLubyte checkImage[checkImageHeight][checkImageWidth][4]; // Texture data
...
private:
    ...
    void loadGlTexture();                          // Load OpenGL texture
    void makeCheckImage();                         // Create check image for texture
};
...
{
    string vertexShaderSource = readShaderSource("vertexShader.vs");
    string fragmentShaderSource = readShaderSource("fragmentShader.fs");
...
void OpenGLApp::display()
{
    ...
    GLfloat squarePositionAngular[12];
    ...
    modelViewMatrix = translate(0.0f, 0.0f, -6.0f); // Translate the square
    ...
    squarePositionAngular[0] = 2.4142f;
    squarePositionAngular[1] = 1.0f;
    squarePositionAngular[2] = -1.41421f;
    squarePositionAngular[3] = 1.0f;
    squarePositionAngular[4] = 1.0f;
    squarePositionAngular[5] = 0.0f;
    squarePositionAngular[6] = 1.0f;
    squarePositionAngular[7] = -1.0f;
    squarePositionAngular[8] = 0.0f;
    squarePositionAngular[9] = 2.4142f;
    squarePositionAngular[10] = -1.0f;
    squarePositionAngular[11] = -1.41421f;

    glBindBuffer(GL_ARRAY_BUFFER, vbo_Position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(squarePositionAngular), squarePositionAngular, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void OpenGLApp::loadGlTexture(void)
{
    // Load the Image
    makeCheckImage();
...
void OpenGLApp::makeCheckImage(void)
{
    int i, j, c;
