...
OpenGLApp::OpenGLApp() : ghwnd(NULL), ghdc(NULL), ghrc(NULL), gpFile(NULL), shaderProgramObject(0),
                         vao(0), vbo_Position(0), vbo_Texture(0), mvpMatrixUniform(0), textureSamplerUniform(0), texture_Spidey(0),
...
int OpenGLApp::initialize(HWND hwnd)
{
    ...
    // create texture
    bResult = loadGlTexture(&texture_Spidey, MAKEINTRESOURCE(MY_SPIDEY_BITMAP));
    if (bResult == FALSE)
    {
        fprintf(gpFile, "Loading Spidey texture Failed\n");
        return -7;
    }
...
void OpenGLApp::display()
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture_Spidey);
