...
private:
    ...
    GLfloat pAngle = 0.0f;                                 // Angle for rotation
...
OpenGLApp::OpenGLApp() : ghwnd(NULL), ghdc(NULL), ghrc(NULL), gpFile(NULL), shaderProgramObject(0),
                         vao(0), vbo_Position(0), vbo_Color(0), mvpMatrixUniform(0), gbFullscreen(FALSE), gbActive(FALSE), dwStyle(0), wpPrev({sizeof(WINDOWPLACEMENT)})
{
...
void OpenGLApp::setupShaders()
{
    ...
    glBindAttribLocation(shaderProgramObject, SM_ATTRIBUTE_COLOR, "aColor");
...
void OpenGLApp::setupBuffers()
{
    ...
    glVertexAttribPointer(SM_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(SM_ATTRIBUTE_COLOR);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void OpenGLApp::display()
{
    ...
    translationMatrix = translate(0.0f, 0.0f, -3.0f); // Translate the pyramid
    scaleMatrix = scale(0.5f, 0.5f, 0.5f);            // Scale the pyramid

    rotationMatrix = rotate(pAngle, 0.0f, 1.0f, 0.0f); // Rotate the pyramid
    ...
    glBindVertexArray(vao);
    glDrawArrays(GL_TRIANGLE_FAN, 0, 12); // Draw the pyramid
